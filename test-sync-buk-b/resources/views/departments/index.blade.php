<x-layouts.app :title="'Departamentos'">
  <div x-data="departmentsPage()" x-init="init()" class="space-y-4">
          ...
          <tbody>
            ...
            <template x-for="d in items" :key="d.id">
              <tr class="border-t">
                ...
                <td class="px-4 py-2" x-text="d.name"></td>
                <td class="px-4 py-2" x-text="d.cost_center_code"></td>
                <td class="px-4 py-2" x-text="formatDate(d.created_at)"></td>
                <td class="px-4 py-2" x-text="formatDate(d.updated_at)"></td>
                <td class="px-4 py-2 text-right">
                  <button @click="open(d.id)"
                          class="px-3 py-1 border rounded hover:bg-gray-50">Ver</button>
                </td>
              </tr>
            </template>
          </tbody>
    ...
    <div x-show="showModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" style="display: none;">
      <div @click.outside="showModal=false" class="bg-white w-full max-w-2xl rounded-lg shadow p-6">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-lg font-semibold">Departamento</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            <label class="block text-sm text-gray-600 mb-1">Centro de costo</label>
            <input type="text" class="w-full border rounded px-3 py-2 bg-gray-100" :value="detail.cost_center_code" disabled>
          </div>
          <div>
  ...
  function departmentsPage(){
    return {
      apiBase: '/api/system-attendance/departments',
      items: [], meta: {}, loading: false,
      showModal: false,
      detail: { id:'', name:'', cost_center_code:'', created_at:'', updated_at:'' },
      ...
      async open(id){
        ...
        this.detail = {
          id: d.id ?? '', name: d.name ?? '', cost_center_code: d.cost_center_code ?? '',
